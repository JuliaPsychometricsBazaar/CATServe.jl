...
function prompt_html(task::PromptedTask)
    return (
        ...
        prompt_html(task.task)
    )
end

function prompt_html(task::SelectMultipleExact)
    return """
        <div id="response">
...
function prompt_html(task::SelectMultiplePartial)
    return partial_checkboxes("answer", answers(task))
end
